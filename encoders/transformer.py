...
class LatticeEncoderLayer(nn.Module):
  ...
  #latt
    ...
    def forward(self, inputs, values):
        ...
        """
   #     input_norm = self.layer_norm(inputs)
        input_norm_split = torch.split(inputs, 256, dim = 2)  # change 512 to 256
   #     value_norm = self.layer_norm(values)
        value_norm_split = torch.split(values, 256, dim = 2)  # change 512 to 256
...
class TransformerEncoder(EncoderBase):
    ...
    def __init__(self, num_layers, d_model, heads, d_ff,
                 dropout, embeddings, embeddings_latt = False):  
...
# latt included within normal transformer, interleaved
         #   if feat_merge == 'latt':
