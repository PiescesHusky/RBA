...
# if lattice is used and number of features is more than 0
...
#latt
# construct embeddings of senses in lattice architecture. 
# the procedure micmics that of embedding of words
        #print('source_backup_latt', source_backup_latt.size())
        if self.feat_merge == 'latt' and len(self.feat_vocab_sizes) > 0:
      #      print('source_backup_latt', source_backup_latt.size())
            if self.position_encoding:
                for i, module in enumerate(self.make_embedding_latt._modules.values()):
                # iterate over modules in make_embedding_latt
              ...
              #      print('self.make_embedding_latt._modules.values()', self.make_embedding_latt._modules.values())
                    if i == len(self.make_embedding_latt._modules.values()) - 1:
                        # to check if 'step' is needed as an argument for module
      ...
      #      print('source_backup_latt final size', source_backup_latt.size())
        ...
        if self.feat_merge == 'latt' and len(self.feat_vocab_sizes) > 0:
           # if lattice is used and there is at least one feature, return embeddings of word and senses.
       #     print('latt produced', source.size(), source_backup_latt.size())
            return source, source_backup_latt
        else:
#latt
         # otherwise, return only embeddings of word, or combined embeddings
        #    print('normal produced', source.size())
            return source
